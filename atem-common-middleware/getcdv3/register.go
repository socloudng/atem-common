...
func RegisterEtcd(schema, etcdAddr, myHost string, myPort int, serviceName string, ttl int) error {
	...
	fmt.Printf("RegisterEtcd %v, %v, %v, %v, %v, %v ", schema, etcdAddr, myHost, myPort, serviceName, ttl)
	if err != nil {
	...
	go func() {
		for {
			...
			case pv, ok := <-kresp:
				if ok == true {
					log.Printf("KeepAlive kresp ok %s", pv)
				} else {
					fmt.Printf("KeepAlive kresp failed %s", pv)
					...
					for {
						...
						resp, err := cli.Grant(ctx, int64(ttl))
						if err != nil {
							log.Panicf("Grant failed %s", err.Error())
							continue
						}

						if _, err := cli.Put(ctx, serviceKey, serviceValue, clientv3.WithLease(resp.ID)); err != nil {
							fmt.Printf("etcd Put failed %v, %v, %v, %v", err.Error(), serviceKey, serviceValue, resp.ID)
							continue
						} else {
							fmt.Printf("etcd Put ok %v, %v, %v", serviceKey, serviceValue, resp.ID)
						}
